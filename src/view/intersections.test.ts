...
describe("line x line", () => {
  it("two geodesics crossing off-center meet at one point", () => {
    ...
    const points = intersectEntities(
      lineB.construction,
      lineB.construction.entities[lineA.id] as Line,
      lineB.construction.entities[lineB.id] as Line,
    );
...
describe("computeIntersectionPoint", () => {
  it("returns the solution at the given branch", () => {
    ...
    const p0 = computeIntersectionPoint(
      circleB.construction,
      circleA.id,
      circleB.id,
      0,
    );
    const p1 = computeIntersectionPoint(
      circleB.construction,
      circleA.id,
      circleB.id,
      1,
  ...
  it("returns null once entities stop intersecting", () => {
    ...
    expect(
      computeIntersectionPoint(circleB.construction, circleA.id, circleB.id, 0),
    ).toBeNull();
  });
  ...
  it("returns null for an unknown or non-intersectable id", () => {
    ...
    expect(
      computeIntersectionPoint(p.construction, "nope", "also-nope", 0),
    ).toBeNull();
