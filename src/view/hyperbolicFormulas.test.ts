...
describe('orthogonalCircleThroughPoints', () => {
  it('computes the center and radius for two symmetric points', () => {
    const circle = orthogonalCircleThroughPoints({ x: 0.5, y: 0 }, { x: 0, y: 0.5 })
  ...
  it('is orthogonal to the unit circle: cx² + cy² = r² + 1', () => {
    ...
    const b = { x: -0.2, y: 0.7 }
    const circle = orthogonalCircleThroughPoints(a, b)!
    expect(circle.cx * circle.cx + circle.cy * circle.cy).toBeCloseTo(circle.r * circle.r + 1)
  })

  it('returns null when A, B and the origin are collinear', () => {
    expect(orthogonalCircleThroughPoints({ x: 0.5, y: 0 }, { x: 0.2, y: 0 })).toBeNull()
    expect(orthogonalCircleThroughPoints({ x: 0.5, y: 0.5 }, { x: -0.3, y: -0.3 })).toBeNull()
  })
})
...
describe('hyperbolicLineThroughPoints', () => {
  it('returns an arc, on the orthogonal circle, that stays inside the disk and passes through A and B', () => {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      const p = { x: circle.cx + shape.r * Math.cos(theta), y: circle.cy + shape.r * Math.sin(theta) }
  ...
  it('returns a diameter when A, B and the origin are collinear', () => {
    const shape = hyperbolicLineThroughPoints({ x: 0.3, y: 0 }, { x: 0.6, y: 0 })
    if (!shape || shape.kind !== 'diameter') throw new Error('expected a diameter')
...
describe('hyperbolicCircleThroughPoints', () => {
  it('computes the center and radius for a hand-checked example', () => {
    const circle = hyperbolicCircleThroughPoints({ x: 0.5, y: 0 }, { x: 0, y: 0.5 })
  ...
  it('stays strictly inside the unit disk', () => {
    const circle = hyperbolicCircleThroughPoints({ x: 0.7, y: 0.2 }, { x: 0.75, y: 0.25 })
    expect(distanceFromOrigin({ x: circle.cx, y: circle.cy }) + circle.r).toBeLessThan(1)
  })
})
