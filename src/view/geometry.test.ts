...
describe('circleShape', () => {
  it('is centered at `center` with radius reaching `thru`', () => {
    expect(circleShape({ x: 10, y: -5 }, { x: 13, y: -1 })).toEqual({ cx: 10, cy: -5, r: 5 })
  })
})
...
describe('clipLineToRect', () => {
  ...
  it('returns null for a line that misses the rect', () => {
    expect(clipLineToRect({ x: -500, y: 200 }, { x: 500, y: 200 }, rect)).toBeNull()
  })
...
describe('lineShape', () => {
  it('renders the clipped line as path data', () => {
    expect(lineShape({ x: -10, y: 0 }, { x: 10, y: 0 }, rect)).toBe('M -100 0 L 100 0')
  })
